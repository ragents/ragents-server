...
function onRequest(request, response) {
  ...
  response.end(getOpts.getHelpHTML())
}

//------------------------------------------------------------------------------
function getFavIcon() {
  if (getFavIcon.pngData) return getFavIcon.pngData

  var pngFile = path.join(__dirname, "ragents.png")

  try {
    getFavIcon.pngData = fs.readFileSync(pngFile)
  }
  catch(e) {
    utils.log("error reading '" + pngFile + "'")
    return null
  }

  return getFavIcon.pngData
}
