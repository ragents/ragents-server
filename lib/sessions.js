...
var DEBUG = debug("ragents:sessions")
...
function Session_onRequest(channel, message) {
  ...
  var reqID = message.to + ":" + message.id

  this.reqCBs.set(reqID, channel)
  agent.channel.sendMessage(message)
}

//------------------------------------------------------------------------------
function Session_onResponse(channel, message) {
  var reqID = message.from + ":" + message.id

  channel = this.reqCBs.get(reqID)

  if (!channel) return

  this.reqCBs.delete(reqID)
